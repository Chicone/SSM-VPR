...
"""This file is to be used instead of ssm.py when the UI is used only to output 
and save the filenames of the  query-recognised files. This is useful for instance
 for reading the pairs into a pose estimation script """

class ssm_MainWindow(ssmbase.Ui_MainWindow):
    """ To be documented by DN..."""
...
#        self.btnRecognition.clicked.connect(self.recognise_places2)
#        self.btnRecognition.clicked.connect(self.recognise_places_sparse) # sparse candidates
    ...
    # def savePath(self):
    #     self.path = qtw.QFileDialog.getExistingDirectory(None, 'Select a folder:', 'C:\\', qtw.QFileDialog.ShowDirsOnly)
    ...
    def refresh_view(self):
        """ 
        Reads all the parameters currently displayed on the GUI and visualises the corresponding signal on the right 
        panel. 
    ...
    def image_visualize_reference(self, img_path):
        ...
        try:
            img_path = '/data/Datasets/RoboCar-Seasons/overcast-reference/rear/1417176458999821.jpg'
    ...
    def crate_db(self):
        ...
        # np.save('db/vectors', np.vstack(projection_acc).astype('float32'))
        ...
        # np.savetxt('db/image_numbers.out', np.hstack(im_numbers_acc), delimiter=',')
        np.savetxt('db/stage1_imgnum_' + self.dataset_name + '_'  + self.imageWidthLineEdit_s1.text() + '_' + self.method + '.out', np.hstack(im_numbers_acc), delimiter=',')
        # np.save('db/vectors_local', np.vstack(projection_geom_acc).astype('float32'))
        np.save('db/stage2_' + self.dataset_name + '_'  + self.imageWidthLineEdit_s2.text(), np.vstack(projection_geom_acc).astype('float32'))
        # np.savetxt('db/image_numbers_local.out', np.hstack(im_numbers_local_acc), delimiter=',')
        np.savetxt('db/stage2_imgnum_' + self.dataset_name + '_'  + self.imageWidthLineEdit_s2.text() + '.out', np.hstack(im_numbers_local_acc), delimiter=',')
        self.textBrowser.append('{} '.format("Database of CNN descriptors created"))
        return 0

    def print_result_pair(self, index, query, recognized, gt):

        print('{:>5}   {:>25}  {:>25}'.format(str(index + 1),
                                                    "Q:" + os.path.splitext(os.path.basename(query))[0],
                                                    "Rec:" + os.path.splitext(os.path.basename(recognized))[0])
                                             )
        if index % 20 == 0:
            self.textBrowser.append('      {:*^5s}   {:>20}  {:>25}'.format("Index", "Query", "Recognized"))

        QApplication.processEvents()

        # vis_str = self.image_visualize_all(fpath,file,gt)
        vis_str = os.path.basename(query) + '_' + os.path.basename(recognized) +  '_' + os.path.basename(gt)
        rgb_str = 'rgb(0, 0, 0)'

        clickable_str = '<html><body>  <a href="http://www.' + vis_str + '"style="text-decoration:none;color:' + rgb_str + '"><pre>' +\
                        '{:>10}   {:>25}  {:>25}'.format((str(index + 1)).center(5),
                                                             (str(int(os.path.splitext(os.path.basename(query))[0]))).center(8), # Query image
                                                             (str(int(os.path.splitext(os.path.basename(recognized))[0][:]))).center(8) ) + \
                        '</pre></a></body></html>'
    ...
    def PrintEvaluation(self, color, index, accuracy, precision, recall, f1, fpath, score, tpi=None, file=None, gt=None):
        ...
        clickable_str = '<html><body>  <a href="http://www.' + vis_str + '"style="text-decoration:none;color:' + rgb_str + '"><pre>' +\
                        '{:>5}  {:>12}  {:>12}  {:>10}  {:>4}  {:>8}  {:>8}  {:>10}  {:>8}'.format((str(index + 1)).center(5),
                                                                                   ...
                                                                                   (str(np.round(f1, 2))).center(6),                        # F1
                                                                                   (str(int(os.path.splitext(os.path.basename(fpath))[0][5:]))).center(8), # Query image
                                                                                   (str(int(os.path.splitext(os.path.basename(file))[0][:]))).center(8),                                  # Recognised image
                                                                                   (str(np.round(score, 1)) + "%").center(10),                # Recognition score
                                                                                   (str(np.round(tpi, 2))).center(8)) + \
                        '</pre></a></body></html>'
    ...
    def baseline_vgg16_s2(self, im, filtered_kernels, im_numbers, nbrs2):
        ...
        vgg16_feature = self.model2.predict(im)
        ...
        ncubes = 0
        for cii in range(self.sblk, h - self.sblk, 2):
            for cjj in range(self.sblk, h - self.sblk, 2):
                ncubes += 1

        # randomly select maximum number of cubes
        if ncubes > self.max_ncubes:
            mask = np.random.choice(ncubes, 2*self.max_ncubes, replace=False)
        else:
        ...
        for cii in range(self.sblk, h - self.sblk, 2):
            for cjj in range(self.sblk, h - self.sblk, 2):
                ...
                block = filter_sel[center[0] - self.sblk: center[0] + self.sblk + 1, center[1] - self.sblk: center[1] + self.sblk + 1]
                block_descrip.append(pp.normalize(block.reshape((2 * self.sblk + 1) ** 2, len(filtered_kernels)), norm='l2', axis=1).flatten())
                cnt2 += 1

        # pca dimensionality reduction
        block_descrip = np.asarray(self.pca_geom.transform(block_descrip))
        ...
        for cii in range(self.lblk, h - self.lblk, 1):
            for cjj in range(self.lblk, h - self.lblk, 1):
                ...
                for rr in range(self.ncand):  # loop through candidates and accumulate statistics
                    try:
                        # file = int(im_numbers[indices[0][rr]])
                        file = int(indices[0][rr] / self.max_ncubes)
                    except:
                        print("Warning: could not get candidate number")
                        pass
                    img_hist_rep[file] += 1  # accumulate repetitions
    ...
    def baseline_vgg16_2(self, im, filtered_kernels, im_numbers, nbrs0):
        """gets the list of candidates (and distances) from IFDB by comparing CNN cubes of the current query"""
        ...
        for cii in range(self.lblk, h - self.lblk, 1):
            for cjj in range(self.lblk, h - self.lblk, 1):
                ...
                img_hist[((indices[0] / self.max_ncubes).astype(int))] += distances[0]

        # img_hist[img_hist == 0] = max(img_hist)

        no_seg = 30
        candidates = []
        ips = int((len(img_hist) - (len(img_hist) % no_seg)) / no_seg) # images per segment
        for c in np.arange(no_seg):
            min_val = min(img_hist[c*ips:(c+1)*ips])
            min_idx = np.where(img_hist == min_val)[0][0]
            for j in np.arange(-6, 6, 3):
                if min_idx + j < 0 or min_idx + j > len(img_hist) -1:
                    continue
    ...
    def baseline_netvlad_2(self, im, sess, net_out, image_batch, vectors, im_numbers, nbrs0):
        ...
        img_hist[ indices[0] ] = distances[0]
        ...
        for c in np.arange(no_seg):
            ...
            #         continue
            #     candidates.append(min_idx + j)
            candidates.append(min_idx)

        # kk = np.where(img_hist_rep != 0)
        # plt.plot(kk[0], list(img_hist[kk]), 'o'); plt.show()
...
#        self.npf = 2                # number of previously recognised places considered when exploiting frame correlation
        ...
        try:
            ...
            # nbrs0 = NearestNeighbors(n_neighbors=self.ncand, algorithm='kd_tree').fit(vectors)  # brute  ball_tree  kd_tree  auto
            # nbrs0 = NearestNeighbors(n_neighbors=self.ncand, algorithm='ball_tree').fit(vectors)  # brute  ball_tree  kd_tree  auto
        ...
        try:
            ...
            # nbrs2 = NearestNeighbors(n_neighbors=self.ncand, algorithm='ball_tree').fit(self.vectors_local[::4] )  # brute  ball_tree  kd_tree  auto
        ...
        if self.method == 'NetVLAD':
            self.no_places = len(im_numbers)
            pass
        elif self.method == 'VGG16':
            self.no_places = int(len(im_numbers) / self.max_ncubes)
        ...
        output2 = open('/data/Datasets/RoboCar-Seasons/S2DHM/results/robotcar/' + self.dataset_name + "_output.txt", "w")
        # output2 = open('/data/Datasets/RoboCar-Seasons/S2DHM/results/robotcar/' + self.dataset_name + "_left_output.txt", "w")
        # output2 = open('/data/Datasets/RoboCar-Seasons/S2DHM/results/robotcar/' + self.dataset_name + "_right_output.txt", "w")
        ...
        for i, fpath in enumerate(filenames):
            ...
            if self.method == 'NetVLAD':
                ...
                # candidates = np.random.choice(np.arange(self.no_places), 1000, replace=False)

            elif self.method == 'VGG16':
                candidates, cand_dist, img_hist = self.baseline_vgg16(im, filtered_kernels, im_numbers, nbrs0)
                # candidates, cand_dist, img_hist = self.baseline_vgg16_s2(im, filtered_kernels, im_numbers, nbrs2)
            ...
            # for c in range(0, self.ncand, 10):
                 cand = candidates[c]
                 if self.method == 'NetVLAD':
                    array_geom_db = self.vectors_local[np.where(self.image_numbers_local == im_numbers[cand])]
                 else:
                    array_geom_db = self.vectors_local[np.where(self.image_numbers_local == im_numbers[cand * self.max_ncubes])]
            ...
            if self.method == 'NetVLAD':
                hit = int(im_numbers[np.argmax(scores_hist)])
            else:
                hit = int(im_numbers[np.argmax(scores_hist) * self.max_ncubes])
...
#            print(np.round(steer*100, 2), '''%''')
            ...
            recog_file = hit
            gt_file = com_path + 'Reference/' + str(filenum) + ext
            recog_file = com_path + 'Reference/'+ str(recog_file) + ext  # create path for recognized image
            ...
            self.print_result_pair(i, fpath, recog_file, gt_file)

            output2.write('rear/' + os.path.basename(fpath) + "," +  'rear/' + os.path.basename(recog_file) + "," + "\n")
            # output2.write('left/' + os.path.basename(fpath) + "," +  'left/' + os.path.basename(recog_file) + "," + "\n")
            # output2.write('right/' + os.path.basename(fpath) + "," +  'right/' + os.path.basename(recog_file) + "," + "\n")
...
#                                  comments="#", delimiter=",", unpack=False)
#
#             nbrs0 = NearestNeighbors(n_neighbors=len(im_numbers), algorithm='brute').fit(vectors)  # brute  ball_tree  kd_tree  auto
#
#             # self.no_places = int(np.max(im_numbers)) + 1
#         except:
#             self.warning_win('Warning', 'Database for current settings not found', 'Please load reference dir and create db')
...
#             if self.method == 'NetVLAD':
#
#                 candidates1, cand_dist, img_hist = self.baseline_netvlad(im, sess, net_out, image_batch, vectors, im_numbers, nbrs0)
#                 candidates2, cand_dist, img_hist = self.baseline_netvlad_2(im, sess, net_out, image_batch, vectors, im_numbers, nbrs0)
#                 # candidates = np.random.choice(np.arange(self.no_places), 1000, replace=False)
#
#             elif self.method == 'VGG16':
#                 candidates1, cand_dist, img_hist = self.baseline_vgg16(im, filtered_kernels, im_numbers, nbrs0)
#                 candidates2, cand_dist, img_hist = self.baseline_vgg16_2(im, filtered_kernels, im_numbers, nbrs0)
#                 # candidates, cand_dist, img_hist = self.baseline_vgg16_s2(im, filtered_kernels, im_numbers, nbrs2)
#
#             candidates = np.concatenate((candidates1[:self.ncand], np.asarray(candidates2)), axis=0)
#
#             # get the query image number from its filename
#             self.img_no = self.get_img_no(os.path.splitext(os.path.basename(fpath))[0])
#
...
#         return accuracy, precision, recall, end0 - start0, np.round(np.mean(np.asarray(time_arr)), 2)
    ...
    def candidates_loop(self, ncand, sparse_factor, candidates, im_numbers, arr_size, hg, wg, vectors_query, nbrs_inst, npf, nlr, i, pf_arr, scores_hist):
        # loop over image filtering candidates
        for c in range(0, ncand, sparse_factor):
            # for c in range(0, self.ncand, 10):
            cand = candidates[c]
            if self.method == 'NetVLAD':
                array_geom_db = self.vectors_local[np.where(self.image_numbers_local == im_numbers[cand])]
    ...
    def recognise_places_sparse(self):
        """performs recognition (stage II) based on the candidates provided by baseline_vgg16 (or baseline_netvlad)"""
...
#        self.npf = 2  # number of previously recognised places considered when exploiting frame correlation
        ...
        output = open(self.dataset + "_output.txt", "a")
        output2 = open('/data/Datasets/RoboCar-Seasons/S2DHM/results/robotcar/' + self.dataset_name + "_output_sparse2000.txt", "w")
        # output2 = open('/data/Datasets/RoboCar-Seasons/S2DHM/results/robotcar/' + self.dataset_name + "_left_output.txt", "w")
        # output2 = open('/data/Datasets/RoboCar-Seasons/S2DHM/results/robotcar/' + self.dataset_name + "_right_output.txt", "w")
        if self.cfg['video']['onscreen'] == True:
            self.show_recog()
        ...
        for i, fpath in enumerate(filenames):
            ...
            sparse_factor = 2
            scores_hist = self.candidates_loop(self.ncand, sparse_factor, candidates, im_numbers, arr_size, hg, wg, vectors_query, nbrs_inst, npf, nlr, i, pf_arr, scores_hist)
            # select highest bin in  histogram as recognised place
            # hit = np.argmax(scores_hist)
            if self.method == 'NetVLAD':
                hit = np.argmax(scores_hist)
            else:
                hit = np.argmax(scores_hist) * self.max_ncubes
            score = np.max(scores_hist)

            # candidates_short = np.arange(4*sparse_factor + 1) - 2*sparse_factor + hit
            candidates_short = np.arange(11) - 5 + hit
            scores_hist = self.candidates_loop(len(candidates_short), 1, candidates_short, im_numbers, arr_size, hg, wg, vectors_query, nbrs_inst, npf, nlr, i, pf_arr, scores_hist)
            if self.method == 'NetVLAD':
                hit = int(im_numbers[np.argmax(scores_hist)])
            else:
                hit = int(im_numbers[np.argmax(scores_hist) * self.max_ncubes])
