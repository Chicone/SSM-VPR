...
### Recognition results
- To save the results of a particular recognition test, press the __Output -> Save__ button. A filename representing the settings currently in the GUI will appear by default. If you want to modify it, edit it and press Accept.
- Pressing the __PR curves__ button opens up a file selection window. Select __one or more files__ an press Accept. A figure will appear showing Precision-Recall curves for each selected set of results. The Area Under the Curve (AUC) is also provided in the legend fir each set. Use the buttons in the figure to interact with it or to save it.
